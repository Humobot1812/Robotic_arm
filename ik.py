...
def forward_kinematics(s0, s1, s2, s3, gripper):
    ...
    x_end = x + L3*np.cos(wrist_angle)*np.sin(t0)
    y_end = y + L3*np.cos(wrist_angle)*np.cos(t0)
...
def inverse_kinematics(x, y, z):
    ...
    theta0 = np.degrees(np.arctan2(x, y))
...
class ArmUI:
    def __init__(self, root):
        ...
        # ===== FK FRAME (ADDED) =====
        self.fk_frame = tk.Frame(root, bg="#0d0d0d")
        self.fk_frame.pack(fill="x")

        self.base = self.slider("BASE", self.fk_frame)
        self.shoulder = self.slider("SHOULDER", self.fk_frame)
        self.elbow = self.slider("ELBOW", self.fk_frame)
        self.wrist = self.slider("WRIST", self.fk_frame)
        self.gripper = self.slider("GRIPPER", self.fk_frame)
        ...
        self.tx = self.slider("X", self.ik_frame)
        self.ty = self.slider("Y", self.ik_frame)
        self.tz = self.slider("Z", self.ik_frame)
        
        self.tx.config(length=500, width=30, sliderlength=40)
        self.ty.config(length=450, width=25)
        self.tz.config(length=450, width=25)

        # override command ONLY for IK sliders
        self.tx.config(command=lambda e: self.update_ik_live())
        self.ty.config(command=lambda e: self.update_ik_live())
        self.tz.config(command=lambda e: self.update_ik_live())
        ...
        # ===== GRIPPER CONTROL IN IK MODE =====
        grip_frame = tk.Frame(self.ik_frame, bg="#0d0d0d")
        grip_frame.pack(pady=5)

        tk.Button(grip_frame, text="OPEN", bg="green", fg="white",
                command=self.open_gripper).pack(side="left", padx=5)

        tk.Button(grip_frame, text="CLOSE", bg="red", fg="white",
                command=self.close_gripper).pack(side="left", padx=5)
    ...
    def open_gripper(self):
        ...
        self.gripper.set(180)   # fully open
        self.update()
    ...
    def close_gripper(self):
        ...
        self.gripper.set(0)     # fully close
        self.update()
    
    
    def slider(self, name, parent=None):
    ...
    def update_ik_live(self):
        ...
        try:
            ...
            # update sliders (this will auto trigger draw)
            self.base.set(s0)
